<template>
  ...
  <el-sub-menu v-if="
    itemData?.children &&
    (itemData.hidden || itemData?.children?.length > 1)
  " :index="itemData.id">
    <template #title>
      <!-- 此处不嵌套el-icon也可正常显示，嵌套了之后可以使用el-menu预设的样式，且在折叠的时候不会闪动 -->
      <el-icon><svg-icon class="menu-icon" :name="itemData.icon"></svg-icon></el-icon>
      <span>{{ itemData.title }}</span>
    </template>
    <!-- <el-menu-item-group> -->
    <sidebar-item v-for="item in itemData.children" :key="item.id" :item-data="item"></sidebar-item>
    <!-- </el-menu-item-group> -->
  </el-sub-menu>
