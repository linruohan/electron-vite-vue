<template>
  <div class="tag-wrap">
    <el-scrollbar ref="scrollPane" class="scroll-pane" @wheel.native.prevent="handleScroll">
      <div class="tag-list">
        <router-link :to="item.path" class="tag-item" :class="{ checked: isCheck(item) }"
          v-for="item in tagsViewStore.visitedViews" ref="tagItem">
          {{ item.name }} &nbsp;&nbsp;
          <svg-icon name="close_window" size="24" @click.prevent="closeTag(item)" />
        </router-link>
      </div>
    </el-scrollbar>
  </div>
...
const initTags = () => {
  const routes = router.getRoutes().sort((a: any, b: any) =>
